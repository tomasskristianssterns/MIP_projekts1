...
FINAL_NUMBER_THRESHOLD = 10

# Skaitļu ģenerēšana (kas dalās ar 2, 3, 4)
def numbers_random_generator():
    numbers = []
    while len(numbers) < 5:
        num = random.randint(LEFT_BOUND, LEFT_BOUND)
...
class Game:
    def __init__(self, number):
        ...
        self.ai_points = 0
        print(f"Game started with number: {self.current_number}")

    def move(self, divisor: int, is_human: bool):
...
# Min Max algoritms izveidots dali ar ChatGPT dali ar savām zināšānām
    def Minimax(self, depth: int, is_maximizing: bool):
        if self.current_number <= 10 or depth == 0:
            print(f"Minimax called: now_number={self.current_number}, depth={depth}, is_max={is_maximizing}")
        ...
        for move in moving_variants:
            if self.current_number % move == 0:
                if move not in [2,3,4]:
                    print(f"Kļūda! Nepareizs dalītājs: {move} priekš {self.current_number}")
                else:
                    print(f"Pareizs gājiens: {self.current_number} ÷ {move} = {self.current_number // move}")
                move_found = True
                new_state = Game(self.current_number // move)  # Izveidojam jaunu spēles stāvokli
...
# Alpha Bet algoritms izveidots dali ar ChatGPT dali ar savām zināšānām
    def Alpha_Beta(self, depth: int, alpha: int, beta: int, is_maximizing: bool):
        if self.current_number <= 10 or depth == 0:
            print(f"Alpha-Beta: now={self.current_number}, depth={depth}, alpha={alpha}, beta={beta}, is_max={is_maximizing}")
            return self.heuristic()
            ...
            for move in moving_variants:
                if self.current_number % move == 0:
                    if move not in [2,3,4]:
                        ...
                        print(f"Pareizs gājiens: {self.current_number} ÷ {move} = {self.current_number // move}")
                    move_found = True
                    new_state = Game(self.current_number // move)
                    new_state.player_points = self.player_points
                    new_state.ai_points = self.ai_points
...
class GameGUI:
  ...
  def start_game(self, player_starts: bool):
  ...
  def player_move(self, divisor: int):
        if self.game.move(divisor, True):
            ...
            self.update_status()
            if self.game.current_number > FINAL_NUMBER_THRESHOLD:
                self.ai_move()
        else:
...
# Simulē AI domāšanu no ChatGPT
  def ai_move(self):
        ...
        for move in [2, 3, 4]:
            if self.game.current_number % move == 0:
                new_state = Game(self.game.current_number)
...
# no ChatGPT
  def update_status(self):
        self.status_label.config(text=f"Skaitlis: {self.game.current_number} | Spēlētāja punkti: {self.game.player_points} | AI punkti: {self.game.ai_points}")
